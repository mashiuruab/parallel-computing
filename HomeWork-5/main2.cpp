...
void linear(int send_buf[],  int length) {
    ...
    if(my_rank == MASTER_RANK) {
        for(int c=1;c<number_of_process;c++){
            MPI_Send(send_buf, length, MPI_INT, c, DEFAULT_CHANNEL, MPI_COMM_WORLD);
        }
    } else {
        MPI_Status status;
        MPI_Recv(recv_buf, length, MPI_INT, MASTER_RANK, DEFAULT_CHANNEL, MPI_COMM_WORLD, &status);
...
void ring(int send_buf[],  int length) {
    ...
    if(my_rank == MASTER_RANK) {
        ...
        MPI_Send(send_buf, length, MPI_INT, process_to_send, DEFAULT_CHANNEL, MPI_COMM_WORLD);
    } else if (my_rank ==  number_of_process - 1) {
        int recv_from = my_rank - 1;
        int recv_buf[length];
        MPI_Status status;
        MPI_Recv(recv_buf, length, MPI_INT, recv_from, DEFAULT_CHANNEL, MPI_COMM_WORLD, &status);
        ...
        int process_to_send = my_rank + 1;
        MPI_Send(recv_buf, length, MPI_INT, process_to_send, DEFAULT_CHANNEL, MPI_COMM_WORLD);
    }
}
...
void double_ring(int send_buf[],  int length) {
    ...
    if(number_of_process <=3) {
        if(number_of_process == 1) {
            ...
            if(my_rank == MASTER_RANK) {
                MPI_Send(send_buf, length, MPI_INT, my_rank + 1,
                         DEFAULT_CHANNEL, MPI_COMM_WORLD);
            } else {
                int recv_buf[length];
                MPI_Status status;

                MPI_Recv(recv_buf, length, MPI_INT, MPI_ANY_SOURCE,
            ...
            if(my_rank == MASTER_RANK) {
                MPI_Send(send_buf, length, MPI_INT, my_rank + 1,
                         DEFAULT_CHANNEL, MPI_COMM_WORLD);
                MPI_Send(send_buf, length, MPI_INT, number_of_process - 1,
                         DEFAULT_CHANNEL, MPI_COMM_WORLD);
            } else {
                int recv_buf[length];
                MPI_Status status;
    ...
    if(my_rank == MASTER_RANK) {
        MPI_Send(send_buf, length, MPI_INT, my_rank + 1, DEFAULT_CHANNEL, MPI_COMM_WORLD);
        MPI_Send(send_buf, length, MPI_INT, number_of_process - 1, DEFAULT_CHANNEL, MPI_COMM_WORLD);
        ...
        MPI_Recv(recv_buf, length, MPI_INT, recv_from,
                 DEFAULT_CHANNEL, MPI_COMM_WORLD, &status);
        ...
        MPI_Send(recv_buf, length, MPI_INT, process_to_send,
                 DEFAULT_CHANNEL, MPI_COMM_WORLD);
...
void tree(int low_rank, int upper_rank, int send_buf[],  int length) {
    ...
    if(my_rank ==  MASTER_RANK) {
        for(int c=0;send_matrix[my_rank][c]!=-1;c++){
            MPI_Send(send_buf, length, MPI_INT, send_matrix[my_rank][c], DEFAULT_CHANNEL, MPI_COMM_WORLD);
        }
    } else if(recv_matrix[my_rank] != -1) {// everyone  should  receive except master
        int recv_buf[length];
        MPI_Status status;
        MPI_Recv(recv_buf, length, MPI_INT, recv_matrix[my_rank],
                 DEFAULT_CHANNEL, MPI_COMM_WORLD, &status);

        print(recv_buf,length, status.MPI_SOURCE);

        for(int c=0;send_matrix[my_rank][c]!=-1;c++){
            MPI_Send(recv_buf, length, MPI_INT, send_matrix[my_rank][c],
                     DEFAULT_CHANNEL, MPI_COMM_WORLD);
        }
    }
}
